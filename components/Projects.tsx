...
import { ArrowUpRight, Car, MapPin, type LucideIcon } from "lucide-react";
...
export type Project = {
  ...
  tech: string[];
  accent: "green" | "cyan" | "purple" | "blue" | "amber" | "rose";
  badge: string;
  images: string[];
  industry: string;
  link?: string;
...
const PRODUCTS_META = [
  ...
  {
    icon: INDUSTRY_ICON.salud,
    statusColor: "text-nexus-blue bg-nexus-blue/10 border-nexus-blue/20",
  ...
  {
    icon: INDUSTRY_ICON.servicios,
    statusColor: "text-nexus-amber bg-nexus-amber/10 border-nexus-amber/20",
...
const WEBS_META = [
  ...
  },
  {
    icon: MapPin,
    statusColor: "text-nexus-blue bg-nexus-blue/10 border-nexus-blue/20",
    tech: ["HTML/CSS", "JavaScript", "Google Maps", "WhatsApp API"],
    accent: "blue" as const,
    images: [],
    industry: "webs",
    link: "https://malvinas-del-norte.vercel.app/",
  },
];
...
const ACCENT_MAP = {
  ...
  blue: { text: "text-nexus-blue", border: "hover:border-nexus-blue/50", glow: "hover:shadow-[0_0_40px_-10px_rgba(59,130,246,0.4)]" },
  amber: { text: "text-nexus-amber", border: "hover:border-nexus-amber/50", glow: "hover:shadow-[0_0_40px_-10px_rgba(245,158,11,0.4)]" },
  rose: { text: "text-nexus-rose", border: "hover:border-nexus-rose/50", glow: "hover:shadow-[0_0_40px_-10px_rgba(244,63,94,0.4)]" },
};
...
export default function Projects() {
  ...
  const filteredProducts = useMemo(
    () => (filter === "all" ? PRODUCTS : PRODUCTS.filter((p) => p.industry === filter)),
          ...
          {t.products.filters.map((f) => (
            <button
              ...
              className={`rounded-full border px-4 py-2 font-mono text-xs transition-colors ${
                filter === f.key
                  ? "border-nexus-green bg-nexus-green/10 text-nexus-green"
